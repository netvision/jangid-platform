...
import { u as useAsyncData, s as superAdminDashboardSchema } from "./index-Cv0Q2NO9.js";
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const pendingProfiles = computed(() => dashboard.value?.pendingProfiles ?? []);
    const signals = computed(() => dashboard.value?.signals ?? []);
    const latestSignups = computed(() => dashboard.value?.latestSignups ?? []);
    const quickActions = computed(() => dashboard.value?.quickActions ?? []);
    const healthChecks = computed(() => dashboard.value?.healthChecks ?? []);
    const activityItems = computed(() => dashboard.value?.recentActivity ?? []);
...
//# sourceMappingURL=index-CdW7TXlN.js.map
